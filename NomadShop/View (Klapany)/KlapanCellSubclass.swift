...
class KlapanCellSubclass: UICollectionViewCell {
    ...
    func setupViews(){
        ...
        if let image = UIImage(named: "wheel") {
            btn_1.setImage(image, for: .normal)
        }
        self.addSubview(btn_1)
        btn_1.snp.makeConstraints { (make) in
            make.top.equalTo(view.snp.top).offset(29.33)
            make.left.equalTo(view.snp.left).offset(29.33)



            make.width.equalTo(53.77)
            make.height.equalTo(53.77)
